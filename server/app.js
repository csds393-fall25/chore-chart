...
app.get('/api/avatar/:userId', async (req, res) => {
  try {
    ...
    for (const part of avatarParts) {
      ...
      try {
        const { data } = supabase.storage.from(bucketName).getPublicUrl(`avatar-props/${part.name}.png`)
        part.url = data.publicUrl;
      } catch (err) {
        console.error('Failed to generate URL for', part.name, err?.message || err);
      }
    }
...
app.get('/api/avatar-props', async (req, res) => {
  try {
    ...
    for (const prop of props) {
      try {
        const { data } = supabase.storage.from(bucketName).getPublicUrl(`avatar-props/${prop.name}.png`)
        prop.url = data.publicUrl;
      } catch (err) {
        console.error('Failed to generate URL for', prop.name, err?.message || err);
      }
    };
    res.json(props);
  } catch (err) {
